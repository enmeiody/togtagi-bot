...
def init_db():
    ...
    migrations = [
        ...
        "ALTER TABLE mijozlar ADD COLUMN til TEXT DEFAULT 'uz'",
        "ALTER TABLE mijozlar ADD COLUMN bloklangan INTEGER DEFAULT 0",
        "ALTER TABLE bronlar ADD COLUMN izoh TEXT",
        ...
        if not m:
            try:
                conn.execute(
                    ...
                    (user_id, ism or "Nomalum", username or "", now, now))
            except Exception:
                conn.execute(
                    "INSERT INTO mijozlar (user_id,ism,username,created_at) VALUES (?,?,?,?)",
                    (user_id, ism or "Nomalum", username or "", now))
            conn.commit()
        else:
            try:
