...
ADMIN_IDS = [8886176055, 7323184602]  # Dinamik, DB dan yuklanadi
...
RESORT_INFO = """
Sen Tog' Tagi Resort ning yordamchi botisan.

Resort haqida ma'lumot:
- Manzil: Shohimardon, Farg'ona viloyati, Ko'lqubondan 300 metr pastda
- Koordinatalar: 39.961311, 71.836921
...
- 5,6,7,8,9,10-xona: 3 kishilik, 300,000 so'm/kecha (2-qavat)
- Jami: 38 ta joy

Xizmatlar:
- Soy bo'yi, Sharshara, Oshxona (mijozlar o'zlari pishiradi)
- Mangal, Shashlik, WiFi, Televizor
- Tapchanlar, Bepul parking, Yashil tabiat

Qoidalar:
- Check-out: ertasi kuni soat 12:00 gacha
- Bron: bot orqali yoki telefon qilib

MUHIM QOIDALAR:
1. FAQAT Resort haqidagi savollarga javob ber
2. Agar savol Resort bilan bog'liq bo'lmasa: "Bu haqida ma'lumotim yo'q. Bog'lanish uchun: +998993342035"
3. Bron qilish so'rasa: "Bron qilish uchun quyidagi tugmani bosing"
4. Javoblar QISQA bo'lsin - 2-3 jumladan oshmasin
5. Foydalanuvchi tilida javob ber (o'zbek/rus/kiril)
"""

def tozala_markdown(matn):
    """AI javobidan markdown belgilarini olib tashlash"""
    import re
...
def ai_javob(savol, til="uz"):
    if not ANTHROPIC_API_KEY:
        return None
    try:
        til_korik = {"uz": "O'zbek tilida javob ber.", "uz_kril": "Ўзбек кирил ёзувида жавоб бер.", "ru": "Отвечай на русском языке."}
        til_txt = til_korik.get(til, "")
        qo_info = get_ai_info()
        extra = ("\n\nQo\'shimcha ma\'lumotlar:\n" + qo_info) if qo_info else ""
        system_txt = RESORT_INFO + extra + " " + til_txt
        data = _json.dumps({
...
def asosiy_menu(uid):
    ...
    kb.add(m["bron"], m["bosh_xonalar"], m["galereya"], m["xizmatlar"], m["manzil"])
    kb.add(types.KeyboardButton(f"📞 {TELEFON1}"))
    return kb
...
def admin_menu():
    ...
    kb.add(
        ...
        "👤 Mijozlar bazasi", "➕ Tezkor bron",
        "📸 Umumiy rasm", "🎥 Video yuklash",
        "📄 Barcha bronlar", "🤖 AI ma'lumot",
        "🔙 Asosiy menyu"
    )
    return kb
...
def start(msg):
    ...
    # Bron havola orqali kelgan: /start bron_A123
    if msg.text and "bron_" in msg.text:
        ...
        if b:
            ...
            # Mijozni bazaga qo'shish (user_id bilan)
            with db() as conn:
                conn.execute(
                    "UPDATE mijozlar SET user_id=? WHERE telefon=?",
                    (uid, b["telefon"]))
                conn.execute(
                    "UPDATE bronlar SET user_id=? WHERE id=?",
                    (uid, bron_id))
                conn.commit()
        else:
            bot.send_message(uid, f"Bron #{bron_id} topilmadi.\nBog'laning: {TELEFON1}")
        return

    # /start bosilganda DOIM til tanlash
    ...
    if not til:
        bot.send_message(uid, "🌐 Tilni tanlang / Выберите язык:", reply_markup=til_menu())
        return
    if msg.text and "/bron" in msg.text: bron_start(msg); return
    if msg.text and "/xonalar" in msg.text: bosh_xonalar_cmd(msg); return
    if msg.text and "/xizmatlar" in msg.text: xizmatlar_cmd(msg); return
    if msg.text and "/manzil" in msg.text: manzil_cmd(msg); return
    if msg.text and "/boglanish" in msg.text: boglanish_cmd(msg); return
    user_state.pop(uid, None)
...
def admin_panel(msg):
    ...
    if is_director(msg.from_user.id):
        bot.send_message(msg.chat.id, "👑 *Director panel*", parse_mode="Markdown", reply_markup=director_menu())
    else:
...
@bot.message_handler(func=lambda m: m.text and (
    "Bog" in m.text and "lanish" in m.text or
    "Боғланиш" in m.text or
    "Контакты" in m.text))
...
def barcha_bronlar(msg):
    with db() as conn:
        bronlar = conn.execute("SELECT * FROM bronlar ORDER BY sana DESC").fetchall()
    if not bronlar:
        bot.send_message(msg.chat.id, "Hozircha bron yo'q", reply_markup=admin_menu())
        return
    matn = f"📄 *Barcha bronlar ({len(bronlar)} ta):*\n\n"
    for b in bronlar[-30:]:
        tasdiq = "✅" if b["tasdiqlangan"] else "⏳"
        matn += (f"{tasdiq} *#{b['id']}* | {b['xona']}\n"
                 f"👤 {b['ism']} | 📞 {b['telefon']}\n"
                 ...
                 f"💰 {format_narx(b['narx'])} so'm\n\n")
    # Uzun bo'lsa bo'laklarga bo'lish
    if len(matn) > 4000:
        bot.send_message(msg.chat.id, matn[:4000], parse_mode="Markdown")
        bot.send_message(msg.chat.id, matn[4000:], parse_mode="Markdown", reply_markup=admin_menu())
    else:
        bot.send_message(msg.chat.id, matn, parse_mode="Markdown", reply_markup=admin_menu())
...
def hozirgi_mehmonlar(msg):
    bugun = datetime.now().strftime("%d.%m.%Y")
    with db() as conn:
        band_xonalar = conn.execute(
            "SELECT DISTINCT bron_id FROM band WHERE sana=? AND bron_id != 'admin'",
            (bugun,)).fetchall()
        mehmonlar = []
        for r in band_xonalar:
            b = conn.execute("SELECT * FROM bronlar WHERE id=? AND tasdiqlangan=1",
                           (r["bron_id"],)).fetchone()
            if b:
                mehmonlar.append(b)
    if not mehmonlar:
        bot.send_message(msg.chat.id, f"👥 Bugun ({bugun}) hech kim yo'q", reply_markup=admin_menu())
        return
    matn = f"👥 *Hozirgi mehmonlar ({bugun}):*\n\n"
    jami_kishi = 0
    for b in mehmonlar:
        bosh_dt = datetime.strptime(b["sana"], "%d.%m.%Y")
        tugash = bosh_dt + timedelta(days=b["kunlar"])
        matn += (f"🎫 *#{b['id']}* | {b['xona']}\n"
                 f"👤 {b['ism']} | 📞 {b['telefon']}\n"
                 f"📅 {b['sana']} → {tugash.strftime('%d.%m.%Y')} ({b['kunlar']} kun)\n"
                 f"👥 {b['kishi']} kishi\n\n")
        jami_kishi += b["kishi"]
    matn += f"📊 Jami: {len(mehmonlar)} xona | {jami_kishi} kishi"
    bot.send_message(msg.chat.id, matn, parse_mode="Markdown", reply_markup=admin_menu())

@bot.message_handler(func=lambda m: m.text == "🤖 AI ma'lumot" and is_admin(m.from_user.id))
def ai_malumot_panel(msg):
    user_state[msg.chat.id] = {"step": "admin_ai_info", "mode": "admin"}
    with db() as conn:
    ...
    if rows:
        mavjud = "\n\n📋 *Mavjud ma'lumotlar:*\n"
        for r in rows:
            mavjud += f"• {r['matn'][:80]}...\n" if len(r['matn']) > 80 else f"• {r['matn']}\n"
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add("🗑 Barcha AI ma'lumotlarni o'chirish", "🔙 Admin menyu")
    bot.send_message(msg.chat.id,
        f"🤖 *AI ma'lumotlar paneli*{mavjud}\n\n"
        "Yangi ma'lumot yozing (masalan: 'Dam olish kunlari: dushanba-yakshanba')\n"
        "Har bir xabar alohida ma'lumot sifatida saqlanadi:",
        parse_mode="Markdown", reply_markup=kb)

@bot.message_handler(func=lambda m: m.text == "🗑 Barcha AI ma'lumotlarni o'chirish" and is_admin(m.from_user.id))
def ai_malumot_tozala(msg):
    with db() as conn:
        conn.execute("DELETE FROM ai_info")
        conn.commit()
    bot.send_message(msg.chat.id, "✅ Barcha AI ma'lumotlari o'chirildi", reply_markup=admin_menu())
    user_state[msg.chat.id] = {"mode": "admin"}

@bot.message_handler(commands=["aiinfo"])
def cmd_aiinfo(msg):
    if not is_admin(msg.from_user.id): return
    matn = msg.text.replace("/aiinfo", "").strip()
    if not matn:
        bot.send_message(msg.chat.id, "Format: /aiinfo [ma'lumot]\nMisol: /aiinfo Chegirma: 20% iyun oyida")
        return
    with db() as conn:
        conn.execute("INSERT INTO ai_info (matn, created_at) VALUES (?,?)",
                    (matn, datetime.now().strftime("%d.%m.%Y %H:%M")))
        conn.commit()
    bot.send_message(msg.chat.id, f"✅ AI ga ma'lumot qo'shildi:\n{matn}")

@bot.message_handler(func=lambda m: m.text == "👮 Adminlar boshqaruvi" and is_director(m.from_user.id))
def adminlar_boshqaruvi(msg):
    with db() as conn:
        adminlar = conn.execute("SELECT * FROM adminlar ORDER BY qoshilgan DESC").fetchall()
    matn = "👮 *Adminlar royxati:*\n\n"
    if adminlar:
        for a in adminlar:
            ism = a["ism"] or "Noma'lum"
            uid_a = a["user_id"]
            sana_a = a["qoshilgan"]
            matn += f"👤 {ism} | ID: {uid_a}\n📅 {sana_a}\n\n"
    else:
        matn += "Hozircha qoshimcha admin yoq\n\n"
    matn += f"👑 *Directorlar:*\n"
    for did in DIRECTOR_IDS:
        matn += f"• {did}\n"
    user_state[msg.chat.id] = {"step": "director_admin_boshqaruv", "mode": "admin"}
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=1)
    kb.add("➕ Admin qo'shish", "❌ Admin o'chirish", "🔙 Admin menyu")
    bot.send_message(msg.chat.id, matn, parse_mode="Markdown", reply_markup=kb)

@bot.message_handler(func=lambda m: m.text == "➕ Admin qo'shish" and is_director(m.from_user.id))
def admin_qoshish(msg):
    user_state[msg.chat.id] = {"step": "director_admin_qosh", "mode": "admin"}
    kb = types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.add("🔙 Admin menyu")
    bot.send_message(msg.chat.id,
        "Yangi admin ID sini kiriting:\n(Admin @userinfobot dan oladi)",
        reply_markup=kb)

@bot.message_handler(func=lambda m: m.text == "❌ Admin o'chirish" and is_director(m.from_user.id))
def admin_ochirish(msg):
    with db() as conn:
        adminlar = conn.execute("SELECT * FROM adminlar").fetchall()
    if not adminlar:
        bot.send_message(msg.chat.id, "O'chirish uchun admin yo'q", reply_markup=director_menu())
        return
    kb = types.InlineKeyboardMarkup(row_width=1)
    for a in adminlar:
        kb.add(types.InlineKeyboardButton(
            f"❌ {a['ism'] or 'Admin'} ({a['user_id']})",
            callback_data=f"del_admin_{a['user_id']}"))
    bot.send_message(msg.chat.id, "❌ *Qaysi adminni o'chirasiz?*",
                     parse_mode="Markdown", reply_markup=kb)
...
def cb_del_admin(call):
    ...
    with db() as conn:
        a = conn.execute("SELECT * FROM adminlar WHERE user_id=?", (uid,)).fetchone()
        conn.execute("DELETE FROM adminlar WHERE user_id=?", (uid,))
        conn.commit()
    ism = a["ism"] if a else str(uid)
    bot.edit_message_text(f"✅ {ism} ({uid}) admin ro'yxatidan o'chirildi",
                          call.message.chat.id, call.message.message_id)
    try:
        bot.send_message(uid, "❌ Sizning admin huquqingiz bekor qilindi.")
    except: pass
    bot.answer_callback_query(call.id)
...
def barcha(msg):
    ...
    # Director - yangi admin qo'shish
    if step == "director_admin_qosh" and is_director(uid):
        try:
            new_admin_id = int(msg.text.strip())
            if new_admin_id in DIRECTOR_IDS:
                bot.send_message(cid, "Bu ID director, alohida admin kerak emas!")
                return
            with db() as conn:
                conn.execute("INSERT OR REPLACE INTO adminlar (user_id, ism, qoshilgan) VALUES (?,?,?)",
                            (new_admin_id, "Admin", datetime.now().strftime("%d.%m.%Y %H:%M")))
                conn.commit()
            bot.send_message(cid, f"✅ {new_admin_id} admin sifatida qo'shildi!", reply_markup=director_menu())
            try:
                bot.send_message(new_admin_id, "✅ Siz Tog' Tagi Resort botiga admin sifatida tayinlandingiz!\n/admin buyrug'ini bosing.")
            except: pass
            user_state[cid] = {"mode": "admin"}
        except ValueError:
            bot.send_message(cid, "Faqat raqam kiriting (Telegram ID)")
        return

    # Admin AI ma'lumot qo'shish
    if step == "admin_ai_info" and is_admin(uid):
        if msg.text and msg.text != "🔙 Admin menyu":
            with db() as conn:
                conn.execute("INSERT INTO ai_info (matn, created_at) VALUES (?,?)",
                            (msg.text, datetime.now().strftime("%d.%m.%Y %H:%M")))
                conn.commit()
            bot.send_message(cid, f"✅ AI ga qo'shildi:\n_{msg.text}_\n\nYana ma'lumot yozing yoki menyuga qayting.",
                           parse_mode="Markdown")
        return
    ...
    if step == "admin_tezkor_telefon" and is_admin(uid):
        ...
        # Mijozga yuborish uchun havola
        havola = f"t.me/togtagi_bot?start=bron_{bron_id}"
        bot.send_message(cid,
            f"Mijozga shu xabarni yuboring:\n\n"
            f"Salom! Tog Tagi Resort da broningiz tasdiqlandi.\n"
            f"Bron ID: #{bron_id}\n"
            f"Xona: {ab['xona_nomi']}\n"
            f"Sana: {ab['sana']} | {ab['kunlar']} kun\n\n"
            f"Batafsil malumot uchun:\n{havola}")

        # Mijozga xabar yuborish (agar bazada bo'lsa)
        try:
            with db() as conn:
                tel_variants = [msg.text, "+998"+msg.text.lstrip("+"), msg.text.lstrip("+998")]
                mijoz = None
                for tel in tel_variants:
                    mijoz = conn.execute("SELECT * FROM mijozlar WHERE telefon=?", (tel,)).fetchone()
                    if mijoz: break
                if not mijoz and len(msg.text) >= 9:
                    oxiri = msg.text[-9:]
                    all_m = conn.execute("SELECT * FROM mijozlar").fetchall()
                    for m2 in all_m:
                        if m2["telefon"] and m2["telefon"][-9:] == oxiri:
                            mijoz = m2
                            break
            if mijoz and mijoz["user_id"]:
                mijoz_uid = mijoz["user_id"]
                til = get_til(mijoz_uid) or "uz"
                bot.send_message(mijoz_uid,
                    t(mijoz_uid, "bron_tasdiqlandi").format(
                        bid=bron_id, xona=ab["xona_nomi"],
                        sana=ab["sana"], kunlar=ab["kunlar"],
                        kishi=ab["kishi"], narx=format_narx(ab["narx"]),
                        tel=TELEFON1),
                    parse_mode="Markdown")
        except Exception as e:
    ...
    # Hech qaysi step ga tushmasa - AI javob beradi
    if not step and msg.text and not msg.text.startswith("/"):
        til = get_til(uid) or "uz"
        javob = ai_javob(msg.text, til)
        if javob:
            # Bron so'rasa tugma ham qo'sh
            if any(x in msg.text.lower() for x in ["bron", "joy", "xona", "заброн", "номер", "бron"]):
                kb = types.InlineKeyboardMarkup()
                kb.add(types.InlineKeyboardButton(t(uid, "bron"), callback_data="bron_start"))
