...
def bosh_menyu(msg):
    user_state.pop(msg.chat.id, None)
    bot.send_message(msg.chat.id, "Bosh menyuga qaytdingiz 👇", reply_markup=asosiy_menu())
...
def admin_holat(msg):
    ...
    matn += "\n*/band 1 15.06.2025* — band qilish\n*/bosh 1 15.06.2025* — bo'sh qilish"
    bot.send_message(msg.chat.id, matn, parse_mode="Markdown", reply_markup=admin_menu())
...
def admin_bron_start(msg):
    ...
    bot.send_message(msg.chat.id, "Mijoz ismi:")
    ...
    try:
        ...
        if not mos:
            bot.edit_message_text(
                f"❌ *{sana}* sanasida mos bo'sh xona yo'q.\n\nBoshqa sana tanlang yoki bog'laning:\n📞 {TELEFON1}",
        ...
        for xid, x, _ in mos[:6]:
            qavat = "🏠 1-qavat" if x["qavat"] == 1 else "🏢 2-qavat"
            narx = format_narx(x["narx"])
            matn += f"{qavat} | {x['nomi']} | {x['sigim']} 👤 | {narx} so'm\n"
        matn += "\n👇 Xonani tanlang:"
        bot.edit_message_text(matn, cid, call.message.message_id, parse_mode="Markdown", reply_markup=xonalar_inline(sana))
...
def cb_xona(call):
    ...
    bot.edit_message_text(
        f"✅ *{x['nomi']}* tanlandi\n"
        f"👥 {x['sigim']} kishilik | 💰 {narx} so'm/kecha\n\n"
...
@bot.callback_query_handler(func=lambda c: c.data in ["tasdiq_ha", "tasdiq_yoq"])
def cb_tasdiq(call):
    cid = call.message.chat.id
    if call.data == "tasdiq_ha":
        ...
        try:
            ...
            bot.send_message(cid, "Bosh menyuga qaytish uchun 👇", reply_markup=asosiy_menu())
        except Exception as e:
            logging.error(e)
            xato_xabar(cid)
    else:
...
def barcha(msg):
    ...
    # ADMIN BRON
    if step == "admin_bron_ism" and msg.from_user.id == ADMIN_ID:
    ...
    if step == "kishi":
        try:
            ...
            kb.add(types.KeyboardButton("🏠 Bosh menyu"))
            bot.send_message(cid, f"✅ {n} kishi\n\n👥 *Kimlar bilan kelmoqchisiz?*", parse_mode="Markdown", reply_markup=kb)
        except ValueError:
    ...
    if step == "guruh":
        ...
        bot.send_message(cid, "📆 30 kunlik sana tanlang:", reply_markup=sana_tugmalari())
        return
